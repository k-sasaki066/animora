...
export default function FoldingMenu() {
    ...
    const scale = width
        ...
        : 1;
    const isMobile = width ? width < 610 : true;

    const { openIndex, setOpenIndex, menuButtonRefs, menuItemRefs, onMenuKeyDown, onMenuItemKeyDown, reduce, mobileOpen, setMobileOpen, toggleMobileMenu } = useFoldingMenu();

    return (
        <nav ref={ref} aria-label="Main menu" className="relative w-full h-full z-0 bg-[#302f2f] overflow-hidden">
            <MobileMenu
                scale={scale}
                reduce={reduce}
                isMobile={isMobile}
