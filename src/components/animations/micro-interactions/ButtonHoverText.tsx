...
export function ButtonHoverText({ className = "w-40 h-12 cursor-pointer", type }: ButtonHoverTextProps) {
    const [isHover, setIsHover] = useState(false);
    const flexCenter = "flex justify-center items-center";
    const animations = {
        ChangeText: (
            <motion.button
                className={`relative px-8 py-4 rounded-sm bg-amber-500 overflow-hidden ${className} ${flexCenter}`}
                initial="rest"
                whileHover="hover"
                animate="rest"
                variants={{
            ...
            >
                ...
                >
                    {/* ["T", "h", "a", "n", "k", "s"] 各文字ごとに motion.span を返す。char: 現在の文字、i: その文字のインデックス */}
        ...
        Flow: (
            <motion.div
                className={`relative group px-8 py-4 rounded-md overflow-hidden bg-green-600 ${className} ${flexCenter}`}
                initial="rest"
                whileHover="hover"
                animate="rest"
            >
        ...
        FancyButton: (
            <motion.div
                className={`relative px-8 py-4 rounded-sm border border-black  overflow-hidden ${className}`}
                // 親のホバーは背景専用にする
                ...
                {/* テキスト */}
                <div className={`relative z-10 w-full h-full space-x-px ${flexCenter}`}>
        ...
        Smoke: (
            <motion.button
                className={`px-8 py-4 rounded-full bg-black text-white font-semibold relative overflow-hidden ${className} ${flexCenter}`}
                whileHover="hover"
                initial="rest"
            >
                {"Button".split("").map((char, i) => (
